...
public class AtomicNetLib {
	...
	public const string kDisconnectionTest  = "disconnectionTest";
	...
	private const float kPingTimeInSeconds = 2.5f;
	private const float kTimeoutForDisconnectionInSeconds = 11f;
	private float _nextPingTime;
	private float _lastDisconnectionTestTime;
	...
	public void Init ()
	{
		...
		_sendMessageQueues.Add (PriorityChannel.ALL_COSTS_CHANNEL, new RingBuffer<NYSUNetworkMessage> (maxQueueLength));
	...
	public void Connect (string ipAddress, int sendPort, int readPort, int udpPort, TBUtils.GenericObjectCallbackType callback)
	{
		try {
			...
			// Set the next rtt test time
			_nextPingTime = kPingTimeInSeconds;
//			_lastDisconnectionTestTime = 
	...
	private void _SendNetworkMessage ()
	{
		while (true) {
			...
			if (_readyToSendMessages) {
				...
				if (_sendMessageQueues[PriorityChannel.ALL_COSTS_CHANNEL].Count > 0) {

					NYSUNetworkMessage msg;
					var dequeued = _sendMessageQueues[PriorityChannel.ALL_COSTS_CHANNEL].TryDequeue (out msg);
...
#region Control Messages
	...
	public void AddToPoolMessage (string pool, string poolType, TBUtils.GenericObjectCallbackType callback)
	{
		...
		SendTCPMessage (netMsg, PriorityChannel.ALL_COSTS_CHANNEL, (string error) => {
...
#region Message Handling
	...
	private void _ReceiveMessage (Dictionary<string, object> netMsg)
	{
		...
		if (netMsg.ContainsKey (kDisconnectionTest)) {
			_lastDisconnectionTestTime = float.Parse (netMsg["time"].ToString ());
			return;
		}
...
//		netMsg.Add (kSendToConnId, currentPool);
		netMsg.Add ("connId", connId);

		SendTCPMessage (netMsg, PriorityChannel.RELIABLE_CHANNEL, (string error) => {
			if (!string.IsNullOrEmpty (error)) {
				Console.WriteLine (error);
				return;
			}
	...
	private void _SendDisconnectionTestMessage ()
	{
		Dictionary<string, object> netMsg = new Dictionary<string, object> ();
		netMsg.Add (kDisconnectionTest, true);
//		netMsg.Add ("time", Time.unscaledTime);
...
//		netMsg.Add (kSendToConnId, currentPool);
	...
	private void _SendPing1 ()
	{
		// Only run the following if we're connected to the relay server
		if (!isConnected || !_readyToSendMessages)
			return;

		// Send a ping every 5 seconds to determine rtt and if tcp is disconnected
		if (_nextPingTime <= 0f) {
			_nextPingTime += kPingTimeInSeconds;
			//TODO Readd check for disconnection when its reliable.
			//_CheckForDisconnection();
			_SendPing();
		} else {
//			_nextPingTime -= Time.unscaledDeltaTime;
		}
	}

	// TODO: Put this on a timer
	private void _CheckForDisconnection ()
	{
		/*

		float secondsSinceLastReceive = (Time.unscaledTime - _lastDisconnectionTestTime) - kPingTimeInSeconds;
		if (secondsSinceLastReceive > kTimeoutForDisconnectionInSeconds) {
			Debug.Log ("We have been disconnected");
			isConnected = false;
		} else {
			_SendDisconnectionTestMessage ();
		} */
	}
